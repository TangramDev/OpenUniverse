	...
	if (pGrid)
	{
		...
		if (h)
		{
			...
			if (lRes)
			{
				...
				{
					CQuasar* pQuasar = pRetNode->m_pGridCommonData->m_pQuasar;
					pQuasar->HostPosChanged();
				}
			}
		}
		...
		default:
		{
			...
			else
			{
				...
				if (m_pGrid->m_pDisp)
				{
					...
					((CGridHelperWnd*)m_pGrid->m_pHostWnd)->m_pGrid = m_pGrid;
					HWND hPage = m_pGrid->m_pGridCommonData->m_pGalaxyCluster->m_hWnd;
					::SendMessage(hPage, WM_COSMOSMSG, (WPARAM)((CGridHelperWnd*)m_pGrid->m_pHostWnd), 1963);
					CComQIPtr<IOleInPlaceActiveObject> pIOleInPlaceActiveObject(m_pGrid->m_pDisp);
					if (pIOleInPlaceActiveObject)
						m_pOleInPlaceActiveObject = pIOleInPlaceActiveObject.Detach();
					m_Wnd.Detach();
