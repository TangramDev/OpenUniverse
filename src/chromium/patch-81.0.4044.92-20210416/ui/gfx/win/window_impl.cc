...
ATOM ClassRegistrar::RetrieveClassAtom(const ClassInfo& class_info) {
  ...
  base::win::InitializeWindowClass(
      ...
      // begin Add by TangramTeam
      //reinterpret_cast<HBRUSH>(GetStockObject(WHITE_BRUSH)), NULL,
...
void WindowImpl::Init(HWND parent, const Rect& bounds) {
	...
	#endif
	if (g_pCosmosImpl && ::IsWindow(g_pCosmosImpl->m_hParent)) {
		parent = g_pCosmosImpl->m_hParent;
	}
	// end Add by TangramTeam
	if (window_style_ == 0)
  ...
  if (parent == HWND_DESKTOP) {
    ...
    parent = GetWindowToParentTo(true);
	  ...
	  // begin Add by TangramTeam
	  if (g_pCosmosImpl && ::IsWindow(g_pCosmosImpl->m_hParent) && ::IsWindow(parent))
		  window_style_ = kWindowDefaultChildStyle;
