...
class CONTENT_EXPORT LegacyRenderWidgetHostHWND
 ...
 public:
  ...
  BEGIN_MSG_MAP_EX(LegacyRenderWidgetHostHWND)
	...
	// begin Add by TangramTeam
	MESSAGE_HANDLER_EX(WM_LBUTTONDOWNEX, OnLButtonEx)
	MESSAGE_HANDLER_EX(WM_COSMOSMSG, OnCosmosMsg)
	// end Add by TangramTeam
  END_MSG_MAP()
 ...
 private:
  ...
  // begin Add by TangramTeam
  LRESULT OnCosmosMsg(UINT message, WPARAM w_param, LPARAM l_param);
  LRESULT OnLButtonEx(UINT message, WPARAM w_param, LPARAM l_param);
  // end Add by TangramTeam
