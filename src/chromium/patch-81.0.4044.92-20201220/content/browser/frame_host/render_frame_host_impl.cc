...
CommonUniverse::CCosmosImpl* g_pCosmosImpl = nullptr;  // 20200108
#endif
...
RenderFrameHostImpl::RenderFrameHostImpl(
  ...
  if (g_pCosmosImpl == nullptr)
  {
      ...
      if (hModule) {
        typedef CommonUniverse::CCosmosImpl*(__stdcall *
                                             GetCosmosImpl)(ICosmos**);
  ...
  #endif
  if (parent_ == nullptr && g_pCosmosImpl)
	  g_pCosmosImpl->m_pCreatingChromeRenderFrameHostBase = this;
  // end Add by TangramTeam
}
...
void RenderFrameHostImpl::DocumentOnLoadCompleted() {
  ...
  if (g_pCosmosImpl) {
	  ...
	  g_pCosmosImpl->OnDocumentOnLoadCompleted(pHost, hWnd, nullptr);
	  CommonUniverse::CChromeBrowserBase* pChromeBrowserBase = g_pCosmosImpl->GetChromeBrowserBase(view->GetHostWindowHWND());
...
void RenderFrameHostImpl::OnCosmosMessage(std::wstring id,
    std::wstring param1,
    std::wstring param2,
    std::wstring param3,
    std::wstring param4,
	...
	if (g_pCosmosImpl)
	{
        ...
        g_pCosmosImpl->m_pCurrentIPCMsg = &IPCInfo;
		::SendMessage(view->AccessibilityGetAcceleratedWidget(), WM_CHROMEIPCMSG, (WPARAM)this, (LPARAM)&IPCInfo);
	}
    if (g_pCosmosImpl)
    {
        g_pCosmosImpl->m_pCurrentIPCMsg = nullptr;
    }
}

void RenderFrameHostImpl::OnCosmosMessage2(std::wstring id,
    std::wstring param1,
    long handle,
    long nID,
    std::wstring param4,
...
void RenderFrameHostImpl::OnCosmosHostIPCMsg(
    ...
    FrameMsg_TANGRAM_HOST_FLOAT_Map mapFloat/* float map*/)
{
    ...
    {
        pSession = g_pCosmosImpl->CreateCloudSession(m_pProxy);
    }
    ...
    if (pSession)
    {
        ...
        if (g_pCosmosImpl->m_pCLRProxy)
        {
            g_pCosmosImpl->m_pCLRProxy->OnCloudMsgReceived(pSession);
        }
        if (m_pProxy)
        {
    ...
    //    if (g_pCosmosImpl->m_pCLRProxy)
    //    {
    //        //g_pCosmosImpl->m_pCLRProxy->OnCloudMsgReceived(&var);
    //    }
    //    if (m_pProxy)
    //    {
    //        CSession* pSession = g_pCosmosImpl->CreateCloudSession(m_pProxy);
...
void RenderFrameHostImpl::SendCosmosMessage(CommonUniverse::IPCSession* var)
{
    ...
    if (it1 != var->m_mapLong.end() && it1->second == 0)
    {
        ...
        {
            m_mapCosmosSession[it2->second.c_str()] = var;
        }
    }
    else
        delete var;
