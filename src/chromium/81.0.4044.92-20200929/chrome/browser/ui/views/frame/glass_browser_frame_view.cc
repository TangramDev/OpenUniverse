...
GlassBrowserFrameView::GlassBrowserFrameView(BrowserFrame* frame,
  ...
  // begin Add by TangramTeam
  if (::GetWindowLongPtr(views::HWNDForWidget(frame), GWL_STYLE) & WS_CHILD) {
  }
  else {
	  minimize_button_ =
...
gfx::Rect GlassBrowserFrameView::GetBoundsForTabStripRegion(
  ...
  //  end_x = std::min(MinimizeButtonX(), end_x);
	int x = CaptionButtonsOnLeadingEdge()
		? (width() - frame()->GetMinimizeButtonOffset()) : 0;
	int end_x = width();
	if (!CaptionButtonsOnLeadingEdge()) {
...
int GlassBrowserFrameView::MinimizeButtonX() const {
  ...
  HWND hwnd = views::HWNDForWidget(frame());
  if (::IsWindow(hwnd) && (::GetWindowLongPtr(hwnd, GWL_STYLE) & WS_CHILD)) {
	  return frame()->GetMinimizeButtonOffset();
  }
  else {
  ...
  flags.setColor(
      ShouldPaintAsActive()
          ...
          : color_utils::AlphaBlend(SkColorSetRGB(0x55, 0x55, 0x55),
                                    titlebar_color, 0.5f));
  // begin Add by TangramTeam
  bool bOnlyOneTab = false;
  HWND hwnd = views::HWNDForWidget(frame());
  if (browser_view()->IsTabStripVisible() &&
	  browser_view()->tabstrip()->tab_count() == 1 && ::IsWindow(hwnd) &&
