...
bool JavaScriptDialogTabHelperDelegateDesktop::IsWebContentsForemost() {
  ...
  if (browser == nullptr)
  {
      content::WebContentsImpl* pWebContentsImpl = static_cast<content::WebContentsImpl*>(web_contents_);
      if (pWebContentsImpl)
      {
          content::RenderFrameHostImpl* pRenderFrameHostImpl = pWebContentsImpl->GetMainFrame();
          if(pRenderFrameHostImpl)
          { 
              content::RenderWidgetHostView* view = pWebContentsImpl->GetTopLevelRenderWidgetHostView();
              if (view)
              {
                  HWND hwnd = views::HWNDForNativeView(view->GetNativeView());
                  if (::IsWindow(hwnd))
                  {
                      browser = (Browser*)::SendMessage(hwnd, WM_COSMOSMSG, 1992, 20200116);
                  }
              }
          }
      }
