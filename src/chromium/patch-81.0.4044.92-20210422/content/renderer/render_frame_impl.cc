...
#include "c:/universework/openuniverse/src/chrome_proxy/blink/core/cosmos.h"
#include "c:/universework/openuniverse/src/chrome_proxy/blink/core/cosmos_compositor.h"
#include "c:/universework/openuniverse/src/chrome_proxy/blink/core/cosmos_control.h"
#include "c:/universework/openuniverse/src/chrome_proxy/blink/core/cosmos_event.h"
#include "c:/universework/openuniverse/src/chrome_proxy/blink/core/cosmos_galaxy.h"
#include "c:/universework/openuniverse/src/chrome_proxy/blink/core/cosmos_node.h"
#include "c:/universework/openuniverse/src/chrome_proxy/blink/core/cosmos_winform.h"
#include "c:/universework/openuniverse/src/chrome_proxy/blink/core/cosmos_xobj.h"
using namespace std;
...
bool RenderFrameImpl::OnMessageReceived(const IPC::Message& msg) {
  ...
  IPC_BEGIN_MESSAGE_MAP(RenderFrameImpl, msg)
    ...
    // begin Add by TangramTeam
    IPC_MESSAGE_HANDLER(TangramRendererIPCMsg, OnCosmosRendererIPCMsg)
    // end Add by TangramTeam
...
// begin Add by TangramTeam
void RenderFrameImpl::OnCosmosRendererIPCMsg(
  ...
  blink::Cosmos* pCosmos = (blink::Cosmos*)GetWebFrame()->GetCosmos();
  blink::CosmosXobj* var = nullptr;

  auto itObj = pCosmos->mapCloudSession_.find(String(strSession.c_str()));
  ...
  if (strID == L"BindCLRObject") {
    if (strSession != L"") {
      pCosmos->mapCloudSession_.insert(String(strSession.c_str()), var);
  ...
  if (strID == L"WINFORM_CREATED") {
        ...
        if (it != pCosmos->m_mapWinForm.end()) {
          ...
          if (form) {
            form->DispatchEvent(*blink::CosmosEvent::Create(
                blink::event_type_names::kCloudformclosed, var));
    ...
    if (itNode != pCosmos->m_mapCosmosNode.end()) {
      ...
      auto itNodeRet = pCosmos->m_mapCosmosNode.find(itreturnnode->second);
      if (itNodeRet != pCosmos->m_mapCosmosNode.end()) {
...
void RenderFrameImpl::SendCosmosMessageEx(
    blink::CommonUniverse::IPCSession& var) {
