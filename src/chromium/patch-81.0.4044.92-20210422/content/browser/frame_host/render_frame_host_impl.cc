...
// begin Add by TangramTeam
  ...
  if (g_pCosmosImpl == nullptr) {
    ...
    if (hModule) {
      ...
      _pCosmosFunction =
          (GetCosmosImpl)GetProcAddress(hModule, "GetCosmosImpl");
...
RenderFrameHostImpl::~RenderFrameHostImpl() {
  ...
  m_mapCosmosSession.erase(m_mapCosmosSession.begin(),
                           m_mapCosmosSession.end());
  // end Add by TangramTeam
}
...
void RenderFrameHostImpl::DocumentOnLoadCompleted() {
  ...
  if (g_pCosmosImpl) {
    ...
    CommonUniverse::CChromeBrowserBase* pChromeBrowserBase =
        g_pCosmosImpl->GetChromeBrowserBase(view->GetHostWindowHWND());
...
CString RenderFrameHostImpl::GetRenderFrameURL(int nType) {
  ...
  switch (nType) {
    case 0: {
      return A2W(GetSiteInstance()->original_url().GetOrigin().spec().c_str());
    } break;
    case 1: {
