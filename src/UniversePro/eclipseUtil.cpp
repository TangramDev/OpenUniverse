...
_TCHAR * checkPathList( _TCHAR* pathList, _TCHAR* programDir, int reverseOrder) {
	...
	result = (_TCHAR *)malloc(bufferLength * sizeof(_TCHAR));
	c1 = pathList;
    while (c1 != NULL && *c1 != _T_ECLIPSE('\0'))
    {
		...
		if (resultLength + checkedLength + 1> bufferLength) {
			...
			result = (_TCHAR *)realloc(result, bufferLength * sizeof(_TCHAR));
		}
...
_TCHAR * concatPaths(_TCHAR** strs, _TCHAR separator) {
	...
	result = (_TCHAR *)malloc((length + 1) * sizeof(_TCHAR));
	result[0] = 0;
	i = -1;
	while (strs[++i] != NULL) {
...
int containsPaths(_TCHAR * str, _TCHAR** paths) {
	...
	buffer = (_TCHAR *)malloc((_tcslen(str) + 2) * sizeof(_TCHAR));
