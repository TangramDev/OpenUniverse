...
int createSharedData(_TCHAR** id, int size) {
	...
	if (id != NULL) {
		*id = (_TCHAR *)malloc(18 * sizeof(_TCHAR));
#ifdef _WIN64
...
int getSharedData(_TCHAR* id, _TCHAR** data) {
	...
	sharedData = (_TCHAR *)MapViewOfFile(handle, FILE_MAP_WRITE, 0, 0, 0);
...
int setSharedData(const _TCHAR* id, const _TCHAR* data) {
	...
	if (mapHandle == NULL) return -1;
	sharedData = (_TCHAR *)MapViewOfFile(mapHandle, FILE_MAP_WRITE, 0, 0, 0);
	if (sharedData == NULL) return -1;
	if (data != NULL) {
