...
LRESULT CMDIParent::OnCosmosMsg(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL&)
{
	...
	case 20210325:
	{
		if (m_bCreateNewDoc)
		{
			...
			::PostMessage(m_hWnd, WM_COSMOSMSG, (WPARAM)m_pActiveMDIChild, 20210326);
		}
	}
	break;
	case 20210324:
...
STDMETHODIMP CGalaxy::Observe(BSTR bstrKey, BSTR bstrXml, IXobj** ppRetXobj)
{
	...
	*ppRetXobj = (IXobj*)m_pWorkXobj;
	...
	if (pOldNode && pOldNode != m_pWorkXobj)
	{
		...
		//if(m_nGalaxyType== GalaxyType::MDIClientGalaxy)
		::PostMessage(m_hWnd, WM_COSMOSMSG, 0, 20210326);
		//else
		//{
		//	for (auto& it : m_mapXobj)
		//	{
...
LRESULT CGalaxy::OnWindowPosChanging(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL&)
{
	...
	if (m_pWorkXobj)
	{
		if (m_pBindingXobj)
		{
			...
			{
				if (m_pMDIParent && m_pMDIParent->m_pHostBrowser->m_pVisibleWebView->m_pGalaxy)
				{
