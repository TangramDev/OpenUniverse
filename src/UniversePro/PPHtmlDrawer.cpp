...
void CPPHtmlDrawer::SetTableOfColors()
{
	...
	SetColorName(TGM_CLUSTER, ::GetSysColor(COLOR_WINDOW)); 
...
HINSTANCE CPPHtmlDrawer::GotoURL(LPCTSTR url, int showcmd /* = SW_SHOW */)
{
    ...
    if ((__int64)result <= HINSTANCE_ERROR)
	{
...
//		sc.nWidth = 0;
	...
	if (MODE_FIRSTPASS != m_nNumPass)
	{
		...
		LONG nWidthTable = m_defStyle.nPadding + (long)cur_table.width.size() - 1;
		for (pos = 0; pos < cur_table.width.size(); ++pos)
			nWidthTable += cur_table.width [pos] + m_defStyle.nPadding;
		rcTable.bottom += m_defStyle.nPadding + (long)cur_table.height.size() - 1;
