...
CWPFObj* CCosmosProxy::CreateWPFControl(IGrid* pGrid, HWND hPWnd, UINT nID)
{
	if (pGrid)
	{
		...
		try
		{
			...
			{
				Universe::Grid^ _pGrid = (Universe::Grid^)theAppProxy._createObject<IGrid, Universe::Grid>(pGrid);
				Universe::Cosmos::m_pFrameworkElementDic[pWpfControlWrapper->m_pUIElement] = _pGrid;
...
void CCosmosProxy::OnItemSelectionChanged(System::Object^ sender, Forms::ListViewItemSelectionChangedEventArgs^ e)
{
	...
	Universe::Grid^ pGrid = Universe::Cosmos::GetGridFromControl(m_pCurrentForm);
	if (pGrid)
	{
...
Object^ CCosmosProxy::InitGrid(IGrid* _pGrid, Control^ pCtrl, bool bSave, CTangramXmlParse* pParse)
{
	...
	Universe::Grid^ pGrid = (Universe::Grid^)theAppProxy._createObject<IGrid, Universe::Grid>(_pGrid);
	if (pGrid)
	{
...
void CCosmosProxy::SelectGrid(IGrid* pGrid)
{
	...
	{
		if (pGrid)
			pObj = theAppProxy._createObject<IGrid, Universe::Grid>(pGrid);
	}
	catch (...)
	{
...
IDispatch* CCosmosProxy::CreateObject(BSTR bstrObjID, HWND hParent, IGrid* pHostNode)
{
	...
	Universe::Grid^ _pGrid = (Universe::Grid^)_createObject<IGrid, Universe::Grid>(pHostNode);
	if (_pObj == nullptr)
	{
	...
	if (_pObj->GetType()->IsSubclassOf(Control::typeid))
	{
		...
		if (pObj != nullptr && pHostNode)
		{
			...
			if (pObj->GetType()->IsSubclassOf(Form::typeid))
			{
				...
				Universe::Cosmos::Fire_OnFormNodeCreated(BSTR2STRING(bstrObjID), (Form^)pObj, _pGrid);
	...
	else
	{
		...
		{
			Universe::Cosmos::m_pFrameworkElementDic[_pObj] = _pGrid;
			try
			{
...
void CCosmosProxy::OnCloudMsgReceived(CSession* pSession)
{
	...
	Universe::Grid^ thisNode = nullptr;
	if (pGrid)
	{
		thisNode = theAppProxy._createObject<IGrid, Universe::Grid>(pGrid);
	}
		...
		if (thisNode != nullptr) {
			...
			if (m_Parse.LoadXml(strXml))
			{
				...
				for (int i = 0; i < nCount; i++)
				{
					...
					if (pRefGrid)
					{
						thisNode[BSTR2STRING(strName)] = theAppProxy._createObject<IGrid, Universe::Grid>(pRefGrid);
					}
				}
			}
		}
...
void CCosmosProxy::CosmosAction(BSTR bstrXml, void* pvoid)
{
	...
	if (strXml != _T(""))
	{
		...
		{
			Universe::Grid^ pWndGrid = (Universe::Grid^)theAppProxy._createObject<IGrid, Universe::Grid>((IGrid*)pvoid);
			if (pWndGrid != nullptr)
			{
				pWndGrid->ActionData = BSTR2STRING(bstrXml);
				Universe::Cosmos::Fire_OnCosmosActionDelegate(pWndGrid, L"");
			}
			return;
		}
...
void CCosmos::OnObserverComplete(HWND hWnd, CString strUrl, IGrid* pRootGrid)
{
	...
	Universe::Grid^ _pRootNode = theAppProxy._createObject<IGrid, Universe::Grid>(pRootGrid);
...
void __stdcall  CGalaxyClusterEvent::OnTabChange(IGrid* sender, int nActivePage, int nOldPage)
{
	...
	Universe::Grid^ pWndGrid = (Universe::Grid^)theAppProxy._createObject<IGrid, Universe::Grid>(sender);
...
{
	//for each (KeyValuePair<String^, TangramAppProxy^>^ obj in Universe::m_pCosmosAppProxyDic)
	//{
...
CTangramWPFObjWrapper::~CTangramWPFObjWrapper(void)
{
	...
	if (Universe::Cosmos::m_pFrameworkElementDic->TryGetValue(m_pUIElement, pGrid))
	{
