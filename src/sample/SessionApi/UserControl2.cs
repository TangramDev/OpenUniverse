...
namespace SessionApi
{
    ...
    {
        private Cosmos.Grid thisNode = null;
        ...
        private void ThisNode_OnBindCLRObjToWebPage(object SourceObj, Cosmos.Wormhole eventSession, string eventName)
        {
            switch (eventName)
            {
                case "OnMyEvent":
        ...
        {
            if (Cosmos.Cosmos.Wormholes.TryGetValue(sender, out Cosmos.Wormhole session))
            {
