...
void RecordLaunchModeHistogram(LaunchMode mode) {
    ...
    if (g_pHubbleImpl && g_pHubbleImpl->m_nAppType == APP_BROWSER_ECLIPSE)
	    return;
  // end Add by TangramTeam
...
// begin Add by TangramTeam
class ChromeBrowserFactory : public CommonUniverse::CHubbleBrowserFactory {
 public:
...
HWND ChromeBrowserFactory::CreateBrowser(HWND hParent, CString strXml) {
  ...
  if (profile_) {
    ...
    if (xml.SetDoc(LPCTSTR(strXml)) && xml.FindNode()) {
      ...
      std::wstring strPopup = xml.GetAttrib(_T("popup"));
      if (strPopup == L"true")
        bPopup = true;
    ...
    if (b == true) {
      if (g_pHubbleImpl && ::IsWindow(hParent))
        g_pHubbleImpl->m_hParent = hParent;
      ...
      g_pHubbleImpl->m_hParent = NULL;
      g_pHubbleImpl->OnSubBrowserWndCreated(hParent, hBrowser);
      browser->window()->Show();
    }
  }
  return hBrowser;
...
bool StartupBrowserCreatorImpl::Launch(Profile* profile,
  ...
  if (!MaybeLaunchApplication(profile)) {
    ...
    // begin Add by TangramTeam
    if (g_pHubbleImpl && g_pHubbleImpl->m_pBrowserFactory == nullptr) {
      ChromeBrowserFactory* pImpl = new ChromeBrowserFactory();
      g_pHubbleImpl->m_pBrowserFactory = pImpl;
      pImpl->profile_ = profile_;
    }
    if ((g_pHubbleImpl && g_pHubbleImpl->m_nAppType == APP_BROWSER) || process_startup == false)
      // end Add by TangramTeam
  ...
  // begin Add by TangramTeam
  if (g_pHubbleImpl && g_pHubbleImpl->m_pBrowserFactory)
  {
	  g_pHubbleImpl->BrowserAppStart();
  }
  // end Add by TangramTeam
#endif  // defined(OS_WIN)
