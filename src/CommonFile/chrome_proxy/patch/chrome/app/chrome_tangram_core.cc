...
__declspec(dllexport) int __stdcall InitApp(bool bSupportCrashReporting) {
  ...
  typedef CommonUniverse::CHubbleMainDllLoader*(__stdcall * SetMainDllLoader)(
      CommonUniverse::CHubbleMainDllLoader * pLoader);

  HMODULE hModule = ::GetModuleHandle(L"universe.dll");
  SetMainDllLoader pSetMainDllLoader = nullptr;
  if (hModule != nullptr) {
    pSetMainDllLoader = (SetMainDllLoader)GetProcAddress(hModule, "SetMainDllLoader");
  }

  if (pSetMainDllLoader != NULL) {
    pSetMainDllLoader(
        (CommonUniverse::CHubbleMainDllLoader*)MakeMainDllLoader());
  }
  return 0;
}

__declspec(dllexport) bool __stdcall IsBrowserModel(bool bSupportCrashReporting) {
	install_static::InitializeFromPrimaryModule();
	base::FilePath user_data_dir;
	if (!chrome::GetDefaultUserDataDirectory(&user_data_dir))
		return false;
