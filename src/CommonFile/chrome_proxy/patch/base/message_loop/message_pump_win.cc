...
void MessagePumpForUI::DoRunLoop() {
  ...
  if (g_pHubbleImpl == nullptr) {
	  ...
	  if (hModule != nullptr) {
		  CommonUniverse::GetHubbleImplFunction GetHubbleImplFunction =
		  ...
		  if (GetHubbleImplFunction != NULL) {
			  ...
			  g_pHubbleImpl = GetHubbleImplFunction(&pHubble);
		  }
	  }
  }

  CommonUniverse::IHubbleDelegate* pDelegate = nullptr;
  if (g_pHubbleImpl && g_pHubbleImpl->m_dwThreadID == ::GetCurrentThreadId())
  {
	  pDelegate = g_pHubbleImpl->m_pHubbleDelegate;
  }
  ...
  for (;;) {
	...
	if (pDelegate) {
		if (g_pHubbleImpl->m_nAppType != APP_BROWSER &&
			g_pHubbleImpl->m_nAppType != APP_ECLIPSE &&
			g_pHubbleImpl->m_nAppType != APP_BROWSER_ECLIPSE &&
			g_pHubbleImpl->m_hMainWnd == 0)
		{
			state_->should_quit = true;
			break;
		}
