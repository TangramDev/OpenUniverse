...
RenderFrameHostImpl::RenderFrameHostImpl(
  ...
  #endif
  if (parent_ == nullptr && g_pHubbleImpl)
	  g_pHubbleImpl->m_pCreatingChromeRenderFrameHostBase = this;
  // end Add by TangramTeam
}
...
RenderFrameHostImpl::~RenderFrameHostImpl() {
  ...
  for (auto it : m_mapHubbleSession)
  {
      ...
      {
          CommonUniverse::CSession* pSession = (CommonUniverse::CSession*)itX->second;
          delete pSession;
      }
      delete it.second;
  }
...
bool RenderFrameHostImpl::OnMessageReceived(const IPC::Message& msg) {
  ...
  IPC_BEGIN_MESSAGE_MAP(RenderFrameHostImpl, msg)
	...
	// begin Add by TangramTeam
	IPC_MESSAGE_HANDLER(TangramFrameHostMsg_Message, OnHubbleMessage)
	IPC_MESSAGE_HANDLER(TangramFrameHostMsg_Message2, OnHubbleMessage2)
	IPC_MESSAGE_HANDLER(TangramHostIPCMsg, OnHubbleHostIPCMsg)
	// end Add by TangramTeam
  IPC_END_MESSAGE_MAP()
...
void RenderFrameHostImpl::DocumentOnLoadCompleted() {
  ...
  if (g_pHubbleImpl) {
	  ...
	  CommonUniverse::CChromeRenderFrameHost* pHost = static_cast<CommonUniverse::CChromeRenderFrameHost*>(this);
	  g_pHubbleImpl->OnDocumentOnLoadCompleted(pHost, hWnd, nullptr);
	  CommonUniverse::CChromeBrowserBase* pChromeBrowserBase = g_pHubbleImpl->GetChromeBrowserBase(view->GetHostWindowHWND());
...
void RenderFrameHostImpl::OnHubbleMessage(std::wstring id,
    std::wstring param1,
    std::wstring param2,
    std::wstring param3,
    std::wstring param4,
	...
	if (g_pHubbleImpl)
	{
        ...
        g_pHubbleImpl->m_pCurrentIPCMsg = &IPCInfo;
		::SendMessage(view->AccessibilityGetAcceleratedWidget(), WM_CHROMEIPCMSG, (WPARAM)this, (LPARAM)&IPCInfo);
	}
    if (g_pHubbleImpl)
    {
        g_pHubbleImpl->m_pCurrentIPCMsg = nullptr;
    }
}

void RenderFrameHostImpl::OnHubbleMessage2(std::wstring id,
    std::wstring param1,
    long handle,
    long nID,
    std::wstring param4,
...
void RenderFrameHostImpl::OnHubbleHostIPCMsg(
    ...
    FrameMsg_TANGRAM_HOST_FLOAT_Map mapFloat/* float map*/)
{
    ...
    {
        pSession = g_pHubbleImpl->CreateCloudSession(m_pProxy);
    }
    ...
    if (pSession)
    {
        ...
        if (g_pHubbleImpl->m_pCLRProxy)
        {
            g_pHubbleImpl->m_pCLRProxy->OnCloudMsgReceived(pSession);
        }
        if (m_pProxy)
        {
    ...
    //    if (g_pHubbleImpl->m_pCLRProxy)
    //    {
    //        //g_pHubbleImpl->m_pCLRProxy->OnCloudMsgReceived(&var);
    //    }
    //    if (m_pProxy)
    //    {
    //        CSession* pSession = g_pHubbleImpl->CreateCloudSession(m_pProxy);
...
{
	WebContents* pContents = delegate_->GetAsWebContents();
	if (pContents) {
		pContents->OpenURL(content::OpenURLParams(
			GURL(LPCTSTR(strUrl)), content::Referrer(),
...
void RenderFrameHostImpl::InsertString(CommonUniverse::IPCSession* var, CString key, CString value)
{
    var->m_mapString[LPCTSTR(key)] = LPCTSTR(value);
}

void RenderFrameHostImpl::InsertLong(CommonUniverse::IPCSession* var, CString key, long value)
{
    var->m_mapLong[LPCTSTR(key)] = value;
}

void RenderFrameHostImpl::Insertint64(CommonUniverse::IPCSession* var, CString key, __int64 value)
{
    var->m_mapint64[LPCTSTR(key)] = value;
}

void RenderFrameHostImpl::InsertFloat(CommonUniverse::IPCSession* var, CString key, float value)
{
    var->m_mapFloat[LPCTSTR(key)] = value;
}

CString RenderFrameHostImpl::GetString(CommonUniverse::IPCSession* var, CString key)
{
...
long RenderFrameHostImpl::GetLong(CommonUniverse::IPCSession* var, CString key)
{
...
__int64 RenderFrameHostImpl::Getint64(CommonUniverse::IPCSession* var, CString key)
{
...
float RenderFrameHostImpl::GetFloat(CommonUniverse::IPCSession* var, CString key)
{
...
void RenderFrameHostImpl::SendHubbleMessage(CommonUniverse::IPCSession* var)
{
    CString strID = _T("");
    ...
    if (it1 != var->m_mapLong.end() && it1->second == 0)
    {
        ...
        {
            m_mapHubbleSession[it2->second.c_str()] = var;
        }
    }
    else
        delete var;
...
void RenderFrameHostImpl::SendHubbleMessage(CommonUniverse::IPCMsg* pMsg) {
	if (pMsg->m_strId.CompareNoCase(_T("OPEN_URL")) == 0) {
        ...
        if (nPos == -1) {
            pContents->OpenURL(content::OpenURLParams(
                GURL(LPCTSTR(strURLs)), content::Referrer(), nDisposition,
                ui::PAGE_TRANSITION_LINK, false /* is_renderer_initiated */));
        }
        while (nPos != -1) {
            CString strUrl = strURLs.Left(nPos);
            pContents->OpenURL(content::OpenURLParams(
                GURL(LPCTSTR(strUrl)), content::Referrer(), nDisposition,
                ui::PAGE_TRANSITION_LINK, false /* is_renderer_initiated */));
