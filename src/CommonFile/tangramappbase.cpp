...
namespace TangramCommon
{
	...
	CTangramFrameWnd::CTangramFrameWnd()
	{
		...
		theAppImpl.m_pUniverseAppProxy->m_nFrameIndex++;
		m_nFrameIndex = theAppImpl.m_pUniverseAppProxy->m_nFrameIndex;
		CString strFrameKey = _T("");
		strFrameKey.Format(_T("%d"), m_nFrameIndex);
		theAppImpl.m_pUniverseAppProxy->m_mapMainFrame[strFrameKey] = this;
		g_pHubble->put_AppKeyValue(CComBSTR(L"newmdtframe"), CComVariant(strFrameKey));
	...
	BEGIN_MESSAGE_MAP(CTangramFrameWnd, CFrameWndEx)
		...
		ON_MESSAGE(WM_HUBBLE_SETAPPTITLE, OnSetAppTitle)
	END_MESSAGE_MAP()
	...
	BOOL CTangramFrameWnd::Create(LPCTSTR lpszClassName, LPCTSTR lpszWindowName, DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, LPCTSTR lpszMenuName, DWORD dwExStyle, CCreateContext* pContext)
	{
		...
		if (theAppImpl.m_pUniverseAppProxy->m_hCreatingView)
		{
			m_pDoc = pContext->m_pCurrentDoc;
			m_hClient = theAppImpl.m_pUniverseAppProxy->m_hCreatingView;
			...
			theAppImpl.m_pUniverseAppProxy->m_hCreatingView = nullptr;
			((CWinAppEx*)AfxGetApp())->SetRegistryBase(m_strRegKey);
			AfxGetApp()->m_pMainWnd = this;
			theAppImpl.m_pUniverseAppProxy->m_hMainWnd = m_hWnd;
			return TRUE;
		}
		((CWinAppEx*)AfxGetApp())->SetRegistryBase(m_strRegKey);
	...
	void CTangramFrameWnd::OnClose()
	{
		...
		((CWinAppEx*)AfxGetApp())->SetRegistryBase(m_strRegKey);
		CFrameWndEx::OnClose();
		if (theAppImpl.m_pUniverseAppProxy->m_mapMainFrame.size() == 0&& afxContextIsDLL)
		{
			AfxOleUnlockApp();
			if (AfxOleCanExitApp())
	...
	void CTangramFrameWnd::OnDestroy()
	{
		if (m_hClient)
		{
			...
			theAppImpl.m_pUniverseAppProxy->m_hClosingFrame = m_hWnd;
			((CWinAppEx*)AfxGetApp())->SetRegistryBase(m_strRegKey);
			AfxGetApp()->m_pMainWnd = this;
			CFrameWndEx::OnClose();
			POSITION nPos = m_dockManager.GetMiniFrames().GetHeadPosition();
		...
		theAppImpl.m_pUniverseAppProxy->m_hClosingFrame = nullptr;
	}
	...
	LRESULT CTangramFrameWnd::OnQueryAppProxy(WPARAM wp, LPARAM lp)
	{
		if (lp)
		{
			...
			default:
				...
				{
					auto it = theAppImpl.m_pUniverseAppProxy->m_mapMainFrame.find(_T("default"));
					if (it != theAppImpl.m_pUniverseAppProxy->m_mapMainFrame.end() && it->second == this)
					{
						theAppImpl.m_pUniverseAppProxy->m_mapMainFrame.erase(it);
						theAppImpl.m_pUniverseAppProxy->m_mapMainFrame[strKey] = this;
					}
				}
				break;
			}
		}
		return (LRESULT)(static_cast<IUniverseAppProxy*>(theAppImpl.m_pUniverseAppProxy));
	}
	...
	CCosmosDocProxy::CCosmosDocProxy()
	{
		m_bCanDestroyFrame = true;
		m_strTangramID = _T("19631222199206121965060119820911");
		m_strAppProxyID = theAppImpl.m_pUniverseAppProxy->m_strProxyID;
		m_strMainFrameID = _T("");
		m_strDocID = _T("");
		m_strHubbleData = _T("");
		m_bDocLoaded = false;

		theAppImpl.m_pUniverseAppProxy->m_pCurDocProxy = this;
	};
	...
	CCosmosDocHelper::CCosmosDocHelper()
	{
		m_pDoc				= nullptr;
		m_pDocument			= nullptr;
		m_strHubbleData	= _T("");
	...
	void CCosmosDocHelper::TangramSerialize(CDocument* _pDoc, CArchive* pAr)
	{
		IUniverseAppProxy* pProxy = (IUniverseAppProxy*)theAppImpl.m_pUniverseAppProxy;
		if (m_pDocument == nullptr&&_pDoc)
		{
			...
			auto it = pProxy->m_mapHubbleDoc.find((LONGLONG)_pDoc);
			if (it != pProxy->m_mapHubbleDoc.end())
				m_pDoc = it->second;
			if (m_pDoc == nullptr)
				return;
			if (pAr == nullptr)
		...
		if (m_pDoc&&pAr)
		{
			if (pAr->IsStoring())
			{
				...
				m_strHubbleData = OLE2T(bstrUIData);
				(*pAr) << m_strTangramID;
				(*pAr) << m_strAppProxyID;
				(*pAr) << m_strAppName;
			...
			else
			{
				...
				m_mapDataMap.Serialize(*pAr);
				m_bDocLoaded = true;
				m_pDoc->put_TemplateXml(m_strHubbleData.AllocSysString());
			}
		}
	}
	...
	{
		if (::IsWindow(hView) && g_pHubble)
		{
			pContext->m_pNewDocTemplate->GetDocString(m_strDocID, CDocTemplate::regFileTypeId);
			auto it = theAppImpl.m_pUniverseAppProxy->m_mapHubbleDoc.find((LONGLONG)pContext->m_pCurrentDoc);
			if (it != theAppImpl.m_pUniverseAppProxy->m_mapHubbleDoc.end())
			{
				m_pDoc = (ICosmosDoc*)it->second;
			}
			else
			{
				m_pDoc = (ICosmosDoc*)theAppImpl.m_pUniverseAppProxy->m_pHubbleImpl->ConnectHubbleDoc((IUniverseAppProxy*)theAppImpl.m_pUniverseAppProxy, (LONGLONG)pContext->m_pCurrentDoc, hView, hParent, m_strDocID);
				theAppImpl.m_pUniverseAppProxy->m_mapHubbleDoc[(LONGLONG)pContext->m_pCurrentDoc] = m_pDoc;
			}
			if (m_pDoc)
				return TRUE;
		}
	...
	BOOL IUniverseAppProxy::InitHubble(void* pVoid)
	{
		...
		if (hModule) {
			...
			GetCosmos _pHubbleFunction;
			_pHubbleFunction = (GetCosmos)GetProcAddress(hModule, "GetCosmos");
			if (_pHubbleFunction != NULL) {
				g_pHubble = _pHubbleFunction();
			}
			typedef CCosmosImpl*(__stdcall * GetCosmosImpl)();
			GetCosmosImpl _pHubbleFunction2;
			_pHubbleFunction2 = (GetCosmosImpl)GetProcAddress(hModule, "GetCosmosImpl");
			if (_pHubbleFunction2 != NULL) {
				m_pHubbleImpl = _pHubbleFunction2();
			}
		}

		if (g_pHubble)
		{
			//theAppImpl.m_pHubble = g_pHubble;
			theAppImpl.m_pUniverseAppProxy = (IUniverseAppProxy*)this; 
			m_pvoid = pVoid;
			m_mapHubbleDoc.clear();
			m_strCreatingFrameTitle = _T("");
					...
					if (strExt != _T(""))
					{
						...
						if (it != m_mapHubbleDocTemplateID.end())
						{
							...
							if (pVoid)
							{
								...
								strXml += s;
								g_pHubble->put_AppKeyValue(CComBSTR(L"defaulttemplate"), CComVariant(strXml));
								//m_pHubbleProxyBase->m_mapValInfo[_T("defaulttemplate")] = CComVariant(strXml);
							}
							...
							if (strFileTypeID != _T("") && strExt != _T("") && pTemplate)
							{
								...
								strExt.MakeLower();
								m_pHubbleImpl->ConnectDocTemplate(strFileTypeID, strExt, pTemplate);

								CString strXml = m_pHubbleImpl->m_strDefaultTemplate;
								CString _strKey = strProxyID + strExt;
								if (strXml.Find(_strKey) == -1)
								{

									CString strPath = m_pHubbleImpl->m_strAppCommonDocPath + _T("Tangramdoctemplate.xml");
			...
			HRESULT hr = S_OK;
#ifndef _WIN64
			//hr = g_pHubble->put_AppKeyValue(CComBSTR(_T("TangramApplicationImpl")), CComVariant((long)(static_cast<CTangramApplicationImpl*>(&theAppImpl))));
#else
			//hr = g_pHubble->put_AppKeyValue(CComBSTR(_T("TangramApplicationImpl")), CComVariant((__int64)(static_cast<CTangramApplicationImpl*>(&theAppImpl))));
#endif
	...
	int IUniverseAppProxy::OnDestroyMainFrame(CString strID, int nMainFrameCount, int nWndType)
	{
			...
			if (it != m_mapMainFrame.end())
			{
				...
				else if (!afxContextIsDLL)
				{
					if (nMainFrameCount == 0)
						return 0;
					if (m_pHubbleWnd == nullptr)
					{
						CComVariant var;
						g_pHubble->get_AppKeyValue(CComBSTR(_T("tangrammsgwnd")), &var);
						HWND hWnd = (HWND)var.llVal;
						m_pHubbleWnd = new CTangramMainWnd();
						m_pHubbleWnd->SubclassWindow(hWnd);
						::VariantClear(&var);
					}
					pApp->m_pMainWnd = m_pHubbleWnd;
				}
				return m_mapMainFrame.size();
			}
		}
		...
		if (nMainFrameCount == 0 && m_pHubbleWnd)
		{
			::SendMessage(m_pHubbleWnd->m_hWnd, WM_DESTROY, 0, 0);
		...
		if (pWnd)
		{
			if (pWnd->IsKindOf(RUNTIME_CLASS(CPaneFrameWnd))|| pWnd->IsKindOf(RUNTIME_CLASS(CMFCToolBar)))
			{
				...
				else
				{
					...
					pWnd2 = (CFrameWnd*)::SendMessage(hParent, WM_COSMOSMSG, 0, 19631965);
					if (pWnd2 == nullptr)
					{
						...
						{
							pWnd = _pWnd;
							pWnd2 = (CFrameWnd*)::SendMessage(::GetParent(hParent), WM_COSMOSMSG, 0, 19631965);
						}
						else
						{
	...
	ICosmosDoc* IUniverseAppProxy::OpenDocument(void* pDocTemplate, CString strFile, BOOL bNewFrame)
	{
		CDocument* pDoc = nullptr;
		ICosmosDoc* _pDoc = nullptr;
		if (pDocTemplate)
		{
			...
			if (pDoc)
				g_pHubble->get_HubbleDoc((LONGLONG)theAppImpl.m_pUniverseAppProxy, (LONGLONG)pDoc, &_pDoc);
		}
		return _pDoc;
	}

	ICosmosDoc* IUniverseAppProxy::CreateNewDocument(LPCTSTR lpszFrameID, LPCTSTR lpszAppTitle, void* pDocTemplate, BOOL bNewFrame)
	{
		CWnd* pFrame = nullptr;
	...
	void IUniverseAppProxy::OnActiveDocument(ICosmosDoc* ActiveDoc, IXobj* pGridInDoc, IXobj* pGridInCtrlBar, HWND hCtrlBar)
	{
	}
	...
	void IUniverseAppProxy::AddDoc(LONGLONG llDocID, ICosmosDoc* pDoc)
	{
		m_mapHubbleDoc[llDocID] = pDoc;
	}

	ICosmosDoc* IUniverseAppProxy::GetDoc(LONGLONG llDocID)
	{
		auto it = m_mapHubbleDoc.find(llDocID);
		if (it != m_mapHubbleDoc.end())
			return it->second;
	...
	ICosmosDoc* IUniverseAppProxy::NewDoc()
	{
		CDocument* pDoc = NULL;
