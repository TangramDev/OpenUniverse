...
CTangramXmlParse* CTangramXmlParse::AddNode(CTangramXmlParse* pParse, CString strNameFix)
{
	if (pParse)
	{
		...
		if (pDoc)
		{
			...
			if (hr == S_OK)
			{
				CTangramXmlParse* pCosmosXmlParse = new CTangramXmlParse(pNode);
				m_aChildElements.push_back(pCosmosXmlParse);
				return pCosmosXmlParse;
			}
		}
	}
	return NULL;
...
CTangramXmlParse* CTangramXmlParse::AddNode(CString name)
{
	...
	{			
		...
		if (pElement != NULL)
		{
			...
			{
				CTangramXmlParse* pCosmosXmlParse = new CTangramXmlParse(pElement);
				m_aChildElements.push_back(pCosmosXmlParse);
				return pCosmosXmlParse;
			}
		}
