...
#define DEFAULT_OS "win32"
#ifdef _WIN64
#define DEFAULT_OS_ARCH "x86_64"
#else
#define DEFAULT_OS_ARCH "x86"
#endif
#define DEFAULT_WS "win32"
...
_TCHAR* findSymlinkCommand( _TCHAR* command, int resolve )
{
    ...
    if (IS_ABSOLUTE(command))
    {
        ...
        cmdPath = (_TCHAR *)malloc( (length + EXTRA) * sizeof(_TCHAR) ); /* add extra space for a possible ".exe" extension */
    ...
    else
    {
        ...
        if (firstDirSeparator( command ) != NULL)
        {
            ...
            cmdPath = (_TCHAR*)malloc( length * sizeof (_TCHAR));
        ...
        else
        {
            ...
            if (path == NULL)
            	path = _T_ECLIPSE("");
            ch = (_TCHAR *)malloc((_tcslen(path) + MAX_PATH_LENGTH + 2) * sizeof(_TCHAR));
...
_TCHAR* findFile( _TCHAR* path, _TCHAR* prefix)
{
...
#ifdef _WIN32
	fileName = (_TCHAR *)malloc( (_tcslen(path) + 1 + _tcslen(prefix) + 3) * sizeof(_TCHAR));
	...
	if(candidate != NULL) {
		result = (_TCHAR *)malloc((pathLength + 1 + _tcslen(candidate) + 1) * sizeof(_TCHAR));
		_tcscpy(result, path);
...
int isFolder(const _TCHAR* path, const _TCHAR* entry) {
	int result = 0;
	struct _stat stats;
	_TCHAR * fullPath = (_TCHAR *)malloc((_tcslen(path) + _tcslen(entry) + 2) * sizeof(_TCHAR));
...
_TCHAR* checkPath( _TCHAR* path, _TCHAR* programDir, int reverseOrder ) 
{
    ...
    workingDir = (_TCHAR *)malloc(cwdLength * sizeof(_TCHAR));
    while ( _tgetcwd( workingDir, cwdLength ) == NULL ){
    	if (errno == ERANGE) {
    		/* ERANGE : the buffer isn't big enough, allocate more memory */
			cwdLength *= 2;
			workingDir = (_TCHAR *)realloc(workingDir, cwdLength * sizeof(_TCHAR));
			continue;
    	} else {
    ...
    buffer = (_TCHAR *)malloc((_tcslen(paths[0]) + _tcslen(paths[1]) + _tcslen(path) + 2) * sizeof(_TCHAR));
    for ( i = 0; i < 2; i++ ) {
