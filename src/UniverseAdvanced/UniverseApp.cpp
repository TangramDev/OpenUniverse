...
LRESULT CALLBACK CUniverse::CosmosMsgWndProc(_In_ HWND hWnd, UINT msg, _In_ WPARAM wParam, _In_ LPARAM lParam)
{
	...
	case WM_DESTROY:
	{
		if (hWnd == g_pCosmos->m_hCosmosWnd)
		{
			...
			{
				//while (g_pCosmos->m_mapWndXobjCollection.size())
				//{
				//	auto it = g_pCosmos->m_mapWndXobjCollection.begin();
				//	delete it->second;
				//}
				//g_pCosmos->m_mapWndXobjCollection.clear();
				...
				case 20210110:
				{
					...
					if (::IsWindow(hWnd))
					{
						...
						if (pCosmosFrameWndInfo)
						{
							...
							case 2:
							{
								auto it = g_pCosmos->m_mapMDIParent.find(::GetParent(pCosmosFrameWndInfo->m_hClient));
										...
										case 3:
										{
											...
											if (itMdiParent != g_pCosmos->m_mapMDIParent.end())
											{
												...
												if (pMDIParent->m_pHostBrowser)
												{
													...
													pMDIParent->m_pHostBrowser->OpenURL(CComBSTR(pMDIParent->m_pGalaxy->m_pWebPageWnd->m_pChromeRenderFrameHost->GetRenderFrameURL(2)), BrowserWndOpenDisposition::SWITCH_TO_TAB, CComBSTR(""), CComBSTR(""));
