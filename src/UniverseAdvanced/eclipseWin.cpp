...
static _TCHAR* findLib( _TCHAR* command ) {
	...
	if (command != NULL) {
		...
		_TCHAR * ProgramDir = getProgramDir();
		 location = (_TCHAR *)malloc( (_tcslen(ProgramDir) + _tcslen( shippedVMDir ) + 1) * sizeof(_TCHAR) );
        _stprintf( location, _T_ECLIPSE("%s%s"), ProgramDir, shippedVMDir );
