...
BOOL CXobjWnd::Create(LPCTSTR lpszClassName, LPCTSTR lpszWindowName, DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID, CCreateContext* pContext)
{
	...
	if (m_pXobj->m_strID.CompareNoCase(TGM_NUCLEUS) == 0)
	{
		...
		if (m_pXobj->m_pParentObj)
		{
			...
			{
				RECT rc = { 0,0,0,0 };
				if (::SendMessage(pMDIParent->m_hWnd, WM_QUERYAPPPROXY, (WPARAM)&rc, 19921989) == 19921989)
				{
...
LRESULT CXobjWnd::OnTabChange(WPARAM wParam, LPARAM lParam)
{
	...
	if (lParam != wParam)
	{
		...
		if (pInfo)
		{
			...
			case 2:
			{
				pMainParent = m_pXobj->m_pXobjShareData->m_pGalaxy->m_pMDIParent;
		...
		if (pGalaxy->m_pParentMDIWinForm && pGalaxy->m_pParentMDIWinForm->m_pClientGalaxy)
		{
			if (pMDIClientObj)
			{
				pGalaxy->m_pParentMDIWinForm->m_pClientGalaxy->m_pBindingXobj = pMDIClientObj;
			}
		}
			...
			if (pMainParent == nullptr || pMainParent->m_bCreateNewDoc == false)
			{
				...
				if (it != g_pCosmos->m_mapBrowserWnd.end())
				{
					...
					if (pBrowser->m_pParentXobj && pBrowser->m_pParentXobj->m_pParentWinFormWnd)
					{
						...
						CGalaxy* pClientGalaxy = (CGalaxy*)g_pCosmos->GetGalaxy(pGalaxy->m_pParentWinForm->m_hMDIClient);
						if (pClientGalaxy)
						{
...
LRESULT CXobjWnd::OnCosmosMsg(WPARAM wParam, LPARAM lParam)
{
	...
	if (wParam == 0 && lParam)//Create CLRCtrl Node
	{
		...
		case 20210202:
		{
			...
			if (m_pXobj->m_pXobjShareData->m_pGalaxy->m_pMDIParent || m_pXobj->m_pXobjShareData->m_pGalaxy->m_pParentMDIWinForm)
			{
				::PostMessage(m_pXobj->m_pXobjShareData->m_pGalaxy->m_hWnd, WM_COSMOSMSG, 0, 20210411);
			}
		}
		break;
		case 20190602:
