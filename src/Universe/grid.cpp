...
BOOL CGrid::Create(DWORD dwStyle, const RECT & rect, CWnd * pParentWnd, UINT nID, CCreateContext * pContext)
{
	...
	if (hWnd == 0)
	{
		hWnd = CreateWindow(L"Cosmos Grid Class", NULL, WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN, rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top, pParentWnd->m_hWnd, (HMENU)nID, AfxGetInstanceHandle(), NULL);
		if (::IsWindow(m_pHostWnd->m_hWnd) == false)
			bRet = m_pHostWnd->SubclassWindow(hWnd);
	}
...
STDMETHODIMP CGrid::put_URL(BSTR newVal)
{
	...
	//	HWND hBrowser = g_pCosmos->m_pBrowserFactory->CreateBrowser(((CGridHelper*)m_pHostWnd)->m_hWnd, strURL);
	//	((CGridHelper*)m_pHostWnd)->m_hFormWnd = hBrowser;
	//	g_pCosmos->m_hParent = NULL;
	//	auto it = g_pCosmos->m_mapBrowserWnd.find(hBrowser);
	//	if (it != g_pCosmos->m_mapBrowserWnd.end())
